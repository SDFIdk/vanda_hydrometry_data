...
public class DatabaseService {
  ...
  public DatabaseService(StationDao stationDao, MeasurementDao measurementDao,
  ...
  public List<Station> getAllStationsByExaminationType(int examinationTypeSc) {
    List<Station> stationsAndMeasurementTypes =
        stationDao.readStationByExaminationTypeSc(examinationTypeSc);
  ...
  public void saveStations(List<Station> stations) {
    ...
    addMeasurementTypes(stations.stream().flatMap(station -> station.getMeasurementTypes().stream())
    ...
    stations.stream().forEach(
        station -> {
          ...
          if (measurementTypes != null) {
            stationDao.insertStationMeasurementTypeRelations(
                measurementTypes.stream().map(mt -> station.getStationId()).toList(),
                measurementTypes);
          }
        }
    );
  ...
  public void saveStation(Station station) {
    ...
    if (measurementTypes != null) {
      stationDao.insertStationMeasurementTypeRelations(
          measurementTypes.stream().map(mt -> station.getStationId()).toList(), measurementTypes);
    }
  }
  ...
  public List<Measurement> getMeasurementHistory(String stationId,
    ...
    return measurementDao.readMeasurementHistory(stationId, measurementPointNumber,
        examinationTypeSc, measurementDatetime);
  }
  ...
  public Measurement getMeasurement(String stationId,
                                    ...
                                    int examinationTypeSc,
                                    OffsetDateTime measurementDatetime) {
    return measurementDao.readCurrentMeasurement(stationId, measurementPointNumber,
        examinationTypeSc, measurementDatetime);
  }
  ...
  public void deleteMeasurement(String stationId, int measurementPointNumber,
    ...
    measurementDao.deleteMeasurementWithHistory(stationId, measurementPointNumber,
        examinationTypeSc, measurementDatetime);
  }
