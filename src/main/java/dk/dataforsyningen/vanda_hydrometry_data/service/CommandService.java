...
public class CommandService implements ApplicationContextAware {
  ...
  public Map<String, CommandInterface> getAllCommandBeans() {
    ...
    Map<String, CommandInterface> allCommands =
        applicationContext.getBeansOfType(CommandInterface.class);

    // Filter the beans to only those annotated with @CommandQualifier
    return allCommands.entrySet().stream()
        .filter(entry ->
            applicationContext.findAnnotationOnBean(entry.getKey(), CommandQualifier.class) != null)
        .collect(Collectors.toMap(entry -> {
          CommandQualifier annotation =
              applicationContext.findAnnotationOnBean(entry.getKey(), CommandQualifier.class);
